...
def calculate_purchasing_plan(total_days, sellers):
  ...
  sellers.insert(0, (0, float('inf')))  # Consider necessary initial bread
  ...
  while d < total_days:
    ...
    if prev and prev != h[0][2] and plan[prev][0][0] >= h[0][1] and plan[prev][-1][1] < h[0][1]+DAYS_FRESH:
      plan[h[0][2]] = plan[prev]
      plan[prev] = []
  ...
  free_bread = INITIAL_BREAD - plan_total[0]
  if free_bread < 0:
    return None
  i = 1
  while free_bread > 0 and sellers_bak[i][1] < DAYS_FRESH:
    x = min(free_bread, plan_total[i])
    plan_total[i] -= x
    free_bread -= x
    i += 1

  return plan_total[1:-1]
